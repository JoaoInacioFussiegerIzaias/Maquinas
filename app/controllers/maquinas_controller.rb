class MaquinasController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @maquina.update(maquina_params)
        format.html { redirect_to @maquina, notice: "Maquina foi adicionada com sucesso" }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to maquinas_path, status: :see_other, notice: "Maquina foi deletada com sucesso" }
  ...
  private
    ...
    def maquina_params
      params.expect(maquina: [ :nome, :modelo ])
    end
end
